...
function DeliveryPersonHome() {
  const [orders, setOrders] = useState([]);

  // Fetch available orders from the backend
  useEffect(() => {
    axios
      .get(`http://10.10.27.17:8086/api/delivery/availableOrders`)
      .then((response) => {
  ...
  const handleAcceptOrder = (orderNumber) => {
    axios
      .post(`http://10.10.27.17:8086/api/delivery/accept`, { orderNumber })
      .then(() => {
        alert(`Order ${orderNumber} accepted!`);
        setOrders((prevOrders) =>
          prevOrders.filter((order) => order.orderNumber !== orderNumber)
      ...
      {orders.map((order) => (
        ...
        >
          <CardContent sx={{ flex: 1 }}>
            ...
            {/* <Typography variant="body2" color="textSecondary">
              Placed on: {format(new Date(order.timestamp), "yyyy-MM-dd HH:mm")}
            </Typography> */}
