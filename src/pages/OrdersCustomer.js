...
const OrdersCustomer = () => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                const response = await axios.get("http://10.10.27.17:8084/api/order/viewOrders/Abi", {
                    params: { userName: "Wick" }, // Replace with the actual username
                });
                const ordersData = response.data.map(order => ({
                    id: order.orderNumber,
                    date: new Date(order.timestamp).toLocaleDateString(),
                    total: order.productInfoList.reduce(
                        (sum, product) => sum + product.price * product.quantity,
                        0
                    ),
                    items: order.productInfoList,
    ...
    return (
        ...
        >
            ...
            <Typography
                variant="h6"
                sx={{ color: "#f06321", textAlign: "center", marginBottom: 2 }}
            >
                "Here you can view order details, check statuses, and stay updated with
                real-time order progress for a seamless experience."
            </Typography>
